...
function parseString(value: unknown, field: string): string {
  if (typeof value !== 'string' || value.length === 0 || value.length > MAX_STRING) {
    throw new Error(`dsh-plugin-desktop: invalid plugin lifecycle ${field}`)
  }
  return value
}
...
function parsePhase(value: unknown): PluginLifecycleFiberPhase {
  ...
  throw new Error('dsh-plugin-desktop: invalid plugin lifecycle Fiber phase')
}
...
function parseEntry(value: unknown): PluginLifecycleEntryView {
  if (!isRecord(value)
    ...
    throw new Error('dsh-plugin-desktop: invalid plugin lifecycle entry')
  }
  ...
  if (typeof value.enabled !== 'boolean'
    ...
    throw new Error('dsh-plugin-desktop: invalid plugin lifecycle entry flags')
  }
  if (value.mutable === (value.protectedReason !== null)) {
    throw new Error('dsh-plugin-desktop: inconsistent plugin lifecycle mutation policy')
  }
...
export function parsePluginLifecycleSnapshot(value: unknown): PluginLifecycleSnapshot {
  if (!isRecord(value)
    ...
    throw new Error('dsh-plugin-desktop: invalid plugin lifecycle snapshot')
  }
  const entries = value.entries.map(parseEntry)
  if (new Set(entries.map(entry => entry.entryId)).size !== entries.length) {
    throw new Error('dsh-plugin-desktop: duplicate plugin lifecycle entry')
  }
...
function parseReceipt(value: unknown): PluginLifecycleReceipt {
  if (!isRecord(value)
    ...
    throw new Error('dsh-plugin-desktop: invalid plugin lifecycle receipt')
  }
  const entryIds = value.entryIds.map(entryId => parseString(entryId, 'receipt entry id'))
  if (new Set(entryIds).size !== entryIds.length) {
    throw new Error('dsh-plugin-desktop: duplicate plugin lifecycle receipt entry')
  }
  return Object.freeze({
    accepted: true,
    action: value.action,
...
async function readResponse(response: Response): Promise<unknown> {
  ...
  try {
    ...
    throw new Error('dsh-plugin-desktop: plugin lifecycle response was not JSON')
  }
  if (!response.ok) {
...
export function createPluginLifecycleApi(
  ...
  const mutate = async (path: string, entryId?: string): Promise<void> => {
    ...
    if (!receipt.rendererReloadRequired) {
      throw new Error('dsh-plugin-desktop: lifecycle receipt omitted the required renderer reload')
    }
    reloadPage()
  }
  return Object.freeze({
