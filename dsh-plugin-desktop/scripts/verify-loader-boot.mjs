...
try {
  ...
  const canvasManifest = fileURLToPath(new URL('../../dsh-plugin-development-canvas/package.json', import.meta.url))
  if (realpathSync(profileRequire.resolve('dsh-plugin-development-canvas/package.json')) !== realpathSync(canvasManifest)) {
    throw new Error('Canvas package manifest did not resolve from the installed launcher')
  }
  await import('dsh-plugin-development-canvas')
