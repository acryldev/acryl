...
export class DesktopRendererHealthGate {
  ...
  begin(timeoutMs: number): Promise<RendererHealthVerdict> {
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
      throw new Error('acryl-desktop: renderer boot timeout must be a positive integer')
    }
    if (this.phase !== 'idle') {
  ...
  stop(cause: unknown = new Error('acryl-desktop: renderer health monitoring stopped')): void {
    if (this.phase !== 'monitoring') return
    this.phase = 'stopped'
    this.clearTimer()
