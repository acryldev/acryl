...
export interface DesktopActions {
  /** Open the already-configured ACRYL terminal for the active profile. */
  openTerminal(): void
...
export class DesktopActionsService extends Service implements DesktopActions {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopActionsBootstrap) {
    ...
    ctx.effect(
      () => () => { this.disposed = true },
      'acryl-desktop: desktop actions lifetime',
    )
  }
  ...
  private assertActive(): void {
    if (this.disposed) throw new Error('acryl-desktop: desktopActions service disposed')
  }
}
