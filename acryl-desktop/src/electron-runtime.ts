...
export function desktopProductVersion(moduleUrl: string = import.meta.url): string {
  ...
  if (value === null || typeof value !== 'object' || typeof (value as { version?: unknown }).version !== 'string') {
    throw new Error('acryl-desktop: package.json has no product version')
  }
...
/** Native adapter used by the ACRYL launcher and owned by its Cordis shell plugin. */
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  beginRendererBootMonitoring(
    ...
    if (this.rendererHealthGate !== undefined) {
      throw new Error('acryl-desktop: renderer boot monitoring already started')
    }
  ...
  schedule(spec: DesktopShellSpec): () => Promise<void> {
    if (this.scheduled !== undefined || this.mountTask !== undefined) {
      throw new Error('acryl-desktop: a native shell generation is already registered')
    }
  ...
  mountScheduled(beforeInteractive?: () => void): Promise<void> {
    ...
    if (spec === undefined) {
      return Promise.reject(new Error('acryl-desktop: the Cordis shell plugin did not register a window'))
    }
    if (this.mountTask === undefined) {
      ...
      this.mountTask = generation.mount(beforeInteractive).then(() => {
        ...
        void this.offerUpdateArtifactCleanup().catch((cause: unknown) => {
          this.logError(`acryl-desktop: failed to resolve update installer cleanup: ${cause instanceof Error ? cause.message : String(cause)}`)
        })
      }).catch((cause: unknown) => {
  ...
  configureTerminal(spec: DesktopTerminalSpec): void {
    if (this.terminalSpec !== undefined) {
      throw new Error('acryl-desktop: terminal profile is already configured')
    }
  ...
  openTerminal(): void {
    try {
      ...
      if (spec === undefined) {
        throw new Error('acryl-desktop: terminal profile is not configured')
      }
      const electronVersion = process.versions.electron
      if (electronVersion === undefined) {
        throw new Error('acryl-desktop: terminal requires the Electron runtime version')
      }
      openDesktopTerminal({
        platform: this.platform,
  ...
  private handleRendererBootVerdict(report: RendererBootReport): void {
    if (report.status === 'failed') {
      ...
      this.logError(`acryl-desktop: renderer boot failed (plugins: ${plugins}): ${error}`)
    }
    let handled = false
    try {
      handled = this.onRendererBoot(report) === true
    } catch (cause) {
      this.logError(`acryl-desktop: failed to persist renderer boot health: ${cause instanceof Error ? cause.message : String(cause)}`)
    }
    if (report.status === 'failed' && !handled) {
      void this.showRendererBootRecovery(report).catch((cause: unknown) => {
        this.logError(`acryl-desktop: failed to show plugin recovery: ${cause instanceof Error ? cause.message : String(cause)}`)
      })
    }
  }
  ...
  private async showRendererBootRecovery(report: Extract<RendererBootReport, { status: 'failed' }>): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      message: 'ACRYL could not load all plugins.',
      detail: `Failed plugins:\n${plugins}\n\n${error}\n\nOpen ACRYL Terminal to update or remove the failing third-party plugin, then restart ACRYL.`,
      buttons: ['Open ACRYL Terminal', 'Restart ACRYL', 'Dismiss'],
      defaultId: 0,
      cancelId: 2,
      noLink: true,
    })
  ...
  private trayCommand(invoke: () => void | Promise<void>): () => void {
    return () => {
      void Promise.resolve().then(invoke).catch((cause: unknown) => {
        this.logError(`acryl-desktop: tray command failed: ${cause instanceof Error ? cause.message : String(cause)}`)
      })
    }
  }
  ...
  private async confirmUpdateDownload(version: string): Promise<boolean> {
    const result = await dialog.showMessageBox({
      type: 'info',
      title: 'ACRYL Update Available',
      message: `ACRYL ${version} is available.`,
  ...
  private async showManualUpdateCheckResult(result: UpdateCheckResult | null): Promise<void> {
    if (result === null) {
      await dialog.showMessageBox({
        ...
        message: 'ACRYL could not check for updates.',
    ...
    if (result.status === 'up-to-date') {
      await dialog.showMessageBox({
        type: 'info',
        title: 'ACRYL Is Up to Date',
        message: 'No newer version of ACRYL is available.',
    ...
    await dialog.showMessageBox({
      type: 'info',
      title: 'ACRYL Update Available',
      message: `ACRYL ${result.latestVersion} is available.`,
  ...
  private async downloadAndOpenUpdate(version: string, signal: AbortSignal): Promise<void> {
    ...
    if (platform === undefined) {
      throw new Error(`acryl-desktop: updates are unavailable on ${this.platform}`)
    }
    ...
    try {
      ...
      this.logError(`acryl-desktop: failed to remember update installer for cleanup: ${cause instanceof Error ? cause.message : String(cause)}`)
    }

    if (platform === 'darwin') {
      const openError = await shell.openPath(artifactPath)
      if (openError !== '') throw new Error(`acryl-desktop: failed to open update disk image: ${openError}`)
      signal.throwIfAborted()
      await dialog.showMessageBox({
        type: 'info',
        title: 'ACRYL Update Downloaded',
        message: `ACRYL ${version} is ready to install.`,
        detail: 'The disk image has opened. Replace ACRYL in Applications, then reopen it.',
        buttons: ['OK'],
        defaultId: 0,
        noLink: true,
      })
    ...
    const result = await dialog.showMessageBox({
      ...
      title: 'ACRYL Update Downloaded',
      message: `ACRYL ${version} is ready to install.`,
      detail: 'Restart ACRYL and run the installer now?',
    ...
    const spec = this.scheduled
    if (spec === undefined) throw new Error('acryl-desktop: no active shell can exit for update installation')
    signal.throwIfAborted()
  ...
  private async performUpdateArtifactCleanup(): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      message: zh
        ? `ACRYL ${artifact.version} 已安装。`
        : `ACRYL ${artifact.version} has been installed.`,
      detail: zh
        ? `是否删除下载的安装包以释放磁盘空间？\n\n${artifact.path}`
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once('spawn', () => {
        ...
        child.once('error', cause => {
          this.logError(`acryl-desktop: update installer failed after launch: ${cause.message}`)
        })
        child.unref()
        resolve()
      })
  ...
  private reportTerminalLaunchError(cause: unknown): void {
    ...
    this.logError(`acryl-desktop: failed to open terminal: ${error.message}`)
    try {
      dialog.showErrorBox('Unable to Open ACRYL Terminal', error.message)
    } catch (dialogCause) {
      this.logError(`acryl-desktop: failed to show terminal error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}`)
    }
  }
  ...
  private reportDiagnosticExportError(cause: unknown): void {
    const error = cause instanceof Error ? cause : new Error(String(cause))
    this.logError(`acryl-desktop: failed to export diagnostics: ${error.message}`)
    try {
      dialog.showErrorBox('Unable to Export Diagnostics', error.message)
    } catch (dialogCause) {
      this.logError(`acryl-desktop: failed to show diagnostics error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}`)
    }
  }
      ...
      { type: 'separator' },
      {
        ...
        click: () => {
          void spec.requestModeChange(nextDesktopShellMode(spec.mode)).catch((cause: unknown) => {
            this.logError(`acryl-desktop: failed to change shell mode: ${cause instanceof Error ? cause.message : String(cause)}`)
          })
        },
      },
      { type: 'separator' },
