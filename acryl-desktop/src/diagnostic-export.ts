...
export function waitForDiagnosticExportWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    timeout = setTimeout(() => {
      settle(
        () => reject(new Error(
          `acryl-desktop: diagnostic export worker timed out after ${String(timeoutMs)}ms`,
        )),
        true,
      )
    }, timeoutMs)
    ...
    worker.once('exit', (code) => {
      settle(
        () => reject(new Error(
          `acryl-desktop: diagnostic export worker exited with code ${String(code)}`,
        )),
        false,
      )
    })
...
export function exportDiagnosticsZip(
  ...
  if (!Number.isSafeInteger(maxEvidenceBytes) || maxEvidenceBytes <= 0) {
    return Promise.reject(new Error('acryl-desktop: diagnostic evidence byte limit must be a positive integer'))
  }
  if (options.appVersion.length === 0 || options.appVersion.length > 512 || /[\0\r\n]/u.test(options.appVersion)) {
    return Promise.reject(new Error('acryl-desktop: diagnostic app version must be a single non-empty line'))
  }
