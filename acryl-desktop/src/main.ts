...
async function start(): Promise<void> {
  ...
  runtime = new ElectronDesktopRuntime(async () => {
    if (shutdown === undefined) {
      throw new Error('acryl-desktop: shutdown coordinator is not ready')
    }
  ...
  try {
    ...
    const ctx = await boot(
      ...
      async (hostCtx) => {
        ...
        hostCtx.effect(
          () => releasePnpmRuntime,
          'acryl-desktop: packaged pnpm runtime PATH',
        )
        if (dshRuntime !== undefined) {
          hostCtx.effect(
            () => releaseDshRuntime,
            'acryl-desktop: packaged dsh runtime PATH',
          )
        }
        hostCtx.effect(
          () => releasePackageResolver,
          'acryl-desktop: profile package resolution',
        )
        ...
        hostCtx.effect(() => () => {
          ...
          pendingSettingsRestart = undefined
        }, 'acryl-desktop: pending Desktop settings restart')
...
function applyIsolatedUserData(): void {
  ...
  process.stderr.write(`${BIN_NAME}: isolated userData ${override}\n`)
}
...
async function run(): Promise<void> {
  ...
  if (process.argv.includes('--export-diagnostics')) {
    try {
      ...
      const message = `acryl-desktop: failed to export diagnostics: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`
