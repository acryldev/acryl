...
function options(calls: CommandCall[], logs: string[] = []): WindowsPackageOptions {
  return {
    ...
    desktopRoot: 'C:\\repo\\acryl-desktop',
    commandShell: 'C:\\Windows\\System32\\cmd.exe',
    builderCli: 'C:\\repo\\node_modules\\electron-builder\\cli.js',
    verifier: 'C:\\repo\\acryl-desktop\\scripts\\verify-win-installer.ts',
...
describe('Windows x64 installer packaging', () => {
  it('checks without credentials, builds an unsigned NSIS target, then verifies it', () => {
    ...
    expect(calls[0]).toEqual({
      ...
      args: [
        ...
        '/s',
        '/c',
        'corepack pnpm --filter acryl-desktop run check:win-package',
      ],
      cwd: 'C:\\repo',
    ...
    expect(calls[2]).toEqual({
      command: 'C:\\Program Files\\nodejs\\node.exe',
      args: ['C:\\repo\\acryl-desktop\\scripts\\verify-win-installer.ts'],
      cwd: 'C:\\repo\\acryl-desktop',
      env: { PATH: 'C:\\Windows\\System32', SAFE_VALUE: 'kept' },
    })
    expect(logs).toEqual([
  ...
  it('checks without credentials, builds an unsigned portable ZIP target, then verifies it', () => {
    ...
    const value = {
      ...options(calls, logs),
      verifier: 'C:\\repo\\acryl-desktop\\scripts\\verify-win-portable.ts',
    }
    ...
    expect(calls[2]?.args).toEqual([
      'C:\\repo\\acryl-desktop\\scripts\\verify-win-portable.ts',
    ])
    expect(logs).toEqual([
