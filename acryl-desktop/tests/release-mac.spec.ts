...
function baseOptions(
  ...
  return {
    env,
    platform: 'darwin',
    desktopRoot: '/repo/acryl-desktop',
    outputDir: '/repo/acryl-desktop/dist/mac-release',
...
describe('macOS release command boundary', () => {
  it('runs checks without credentials, then gives credentials only to the DMG builder', () => {
    ...
    expect(calls[0]).toEqual({
      command: 'corepack',
      args: ['pnpm', 'run', 'check'],
      cwd: resolve('/repo/acryl-desktop', '..'),
    ...
    expect(calls[1]).toEqual({
      ...
      args: [
        ...
        '--config.directories.output=/repo/acryl-desktop/dist/mac-release',
      ],
    ...
    expect(calls[2]).toEqual({
      ...
      args: [
        ...
        '/repo/acryl-desktop/dist/mac-release',
      ],
      cwd: '/repo/acryl-desktop',
      env: { PATH: '/usr/bin', SAFE_BUILD_VALUE: 'kept' },
    })
  ...
  it('does not invoke electron-builder after a failed credential-free check', () => {
    ...
    expect(calls[0]?.cwd).toBe(resolve('/repo/acryl-desktop', '..'))
