...
import type {
  ...
  DesktopProfiles,
} from 'acryl-desktop/profile-service'
import type {
  DesktopPnpm,
  DesktopPnpmHandle,
  DesktopPnpmOutcome,
} from 'acryl-desktop/pnpm'
```

`acryl-desktop/profiles` is the Desktop-owned tray consumer, not the profile service contract. Do not import it for service types.
...
| --- | --- | --- |
| `desktopProfiles` | Generation-scoped Host service. | Public and supported through `acryl-desktop/profile-service`. |
| `desktopPnpm` | Generation-scoped Host service. | Public and supported through `acryl-desktop/pnpm`. |
...
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'acryl-desktop/profile-service'
import type {} from 'acryl-desktop/pnpm'
...
Type-only imports are erased from JavaScript. A cross-environment package can keep `acryl-desktop` as a development dependency for compilation, or as an optional peer if it publishes declarations that expose these types; it does not need a runtime import merely to probe the services.
...
```sh
pnpm --filter acryl-desktop run build
pnpm --filter acryl-desktop run verify:profile
```
