{
  ...
  outputs = { self, nixpkgs, nixpkgs-darwin-legacy, systems, ... }:
    let
      ...
      # Prebuilt CLI release tarballs (v0.1.36). Each tarball bundles its
    ...
    in
    {
      packages = forAllSystems ({ pkgs, system }:
        ...
        // pkgs.lib.optionalAttrs (prebuiltAssets ? ${system}) {
