...
**Created**: 2026-08-23
**Status**: Draft
**Input**: Orientation spec §26-§31; constitution v1.0.0; gap analysis §2.1 (`ctx.acrRoom` is one of the three genuine ACRYL seams). User directive: build the multi-agent foundation (room + provider seam) before handoff, accounts, and self-evolution.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Open a persistent project room (Priority: P1)

A developer opens ACRYL, creates (or reopens) a project room for a folder, and gets a stable room identity that stays the same across agent sessions and app restarts. The room, not any single agent, owns the project's working context.

**Why this priority**: This is the substrate every later ACRYL capability (providers, handoff, accounts, self-evolution) attaches to. Without a durable room, "agents come and go, work continues" is impossible.
...
1. **Given** a folder without a room, **When** the user opens it in ACRYL, **Then** a room is created with a stable identity bound to that folder.
...
**Why this priority**: This is the ACRYL inversion - agents are disposable actors, the room is the persistent scene. It is the acceptance test for room identity being agent-independent.
...
- Reopening a folder whose room state was deleted: ACRYL re-creates or reports cleanly, never half-opens.
- Disabling/unloading the ACRYL room plugin: durable state on disk is not deleted; re-enabling restores it.
...
- **FR-001**: ACRYL MUST present project rooms as an activatable plugin surface, not a hardwired core path; the room plugin can be enabled and disabled independently.
...
- **FR-008**: A room MUST be able to hold heterogeneous member handles (native or external agents) as peers, without being owned by any one member. Identity only; provider behavior is ACRYL-2.
...
- **RoomMember**: a handle describing a participant (agent or human) attached to the room; provider-backed in ACRYL-2.
...
- The room is the ACRYL product layer on top of DSH, not a new event store. It composes DSH session/persistence seams; it does not duplicate them.
...
- The DSH-native agent and external PTY agents attach to the room in ACRYL-2; this slice only establishes identity, durable state, and a member-handle shape.

## Notes

- Gap analysis §2.1 identifies `ctx.acrRoom` as one of the three genuine ACRYL seams (identity independent of any single agent/session). This spec is that seam.
