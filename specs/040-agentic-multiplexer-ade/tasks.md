...
- [x] **T074** Surface adapters for the parts that differ, as typed seams with a Desktop and a Web implementation: add-project folder chooser (Web: server-side directory browser or path entry with validation), folder drag-drop, window chrome. Replace the DOM-click workaround in `projects-control.ts` with the adapter. *Partly done 2026-09-26: the shell environment and native chrome metrics are seams (Desktop markers versus web); adding a project already uses the window picker where it exists and the upstream flow elsewhere, which browses server folders on Web. Still to do: replace the DOM-click fallback in `projects-control.ts` with a typed adapter, and confirm the Web folder browser end to end.* *Finished 2026-09-26: adding a project is a typed `FolderChooserKind` seam (`picker` where the window offers one, `upstream` for desktop without a picker, `path` on Web, where the sidebar shows a path form and the Host confirms the folder is a git worktree). The upstream-flow fallback stays as its own named adapter for macOS desktop, where it drives the native chooser. Not confirmed in a real browser.*
...
- [x] **T110** Run button and per-worktree run script. Covered by the Checks tab, which already runs a worktree's own package scripts in a terminal tab; a separate configurable run command is not added (it would need the same trust design as custom agents).
...
- [x] **T112** `git/service.ts` (519 lines) is becoming a god service: repo, status, diff, checks, worktree creation, stage, commit and search in one class. Split by use case into bounded modules (for example `git/changes`, `git/commit`, `search`) behind one shared, confined command runner. Delivered (`2613e60`): the git service is split into worktrees, changes, commits and search modules over one confined runner, behind an unchanged facade (the largest file is now 135 lines).
- [x] **T113** Ubiquitous language: "session" means both a chat session (`SessionSummary`) and a terminal's Host process (`tile.sessionId`, `WorkspacePtyRegistry` sessions). Rename the terminal one to "terminal" (`terminalId`) with a migration for saved tabs. Delivered for the client: a tab's terminal handle is `terminalId`. The Host's internal 'session' wording for a terminal process is unchanged and stays inside the Host.
...
**Still open**: the manual browser passes (T076, T096, T105), the attention queue (needs a real "needs approval" signal), cost and context pressure in the status line (needs a verified projection shape), T082 and T083 (Web diagnostics, Blends on Web), and T084 (one loopback-http library for the three copies; touches packaging, so it needs its own careful pass).
