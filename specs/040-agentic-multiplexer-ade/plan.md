...
**Target Platform**: same as `specs/015` — Desktop advanced mode, macOS/Windows first. Web parity is a **requirement**, decided 2026-09-26 (see the spec's "Surface sharing" section): the workspace is composed once through `coding-capabilities.ts` with `surfaces: ['desktop', 'web']`. The earlier hedge that follows is superseded. is not separately gated in this plan; the Client-side pieces (rail, panel, tiles) are plain React/Cordis-Client and should work on Web wherever the Canvas package itself already does.
