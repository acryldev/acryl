...
- [ ] T004 Create `acryl-harness-runtime/src/engine/engine.ts`: `AcrylEngineName`, `AcrylRuntimeMarker` (+ `declare module '@deepseek-ai/cordis'` for `ctx.runtime`), `AcrylEngineHandle`, `AcrylEngineStartOptions`, `AcrylEngineAdapter` (with `fidelity`), `AcrylEngineError` + `AcrylEngineErrorCode`, per [contracts/engine-runtime.md](./contracts/engine-runtime.md). Types only. Proof: `corepack pnpm run typecheck`.
