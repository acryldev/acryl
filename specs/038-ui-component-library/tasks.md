...
- **T034 Move the gallery out of Settings.** Remove the `acryl-example-ui-library` Settings tab from the default experience. It stays as an example in the pack (`example-plugins/packages/client-ui-library`, doc `extending.ui-library`) that a person runs on demand (`/reload new` from the examples list) or an agent installs when asked. Update `docs/extending/ui-library.md` and the pack tests. Done when: a fresh profile has no "UI library" Settings tab, the example still installs and renders, and the doc says how to open it.
...
- **T036 Registry format and repo.** Create `acryl-ui-registry` (git repo, hub form D6): `registry/<id>/item.yaml` plus source, generated `index.json` (kind `ui-component`), a validate command reusing the blends ingest shape. Seed with the existing 12 components and their provenance. Done when: the repo validates, the index regenerates identically twice, and a bad item (hex color, no origin) is rejected.
...
- **T038 `acryl ui add`.** In `acryl-cli`, plus `acryl ui list`, `acryl ui diff`. Reads a registry (git URL or local path), verifies the digest, writes source into the plugin, records `ui.lock.json`. Done when: a scaffolded plugin adds `acryl.ui.card`, builds, and renders in a real browser with no `acryl-ui-web` import.
...
- **T041 Publish `@acryl/ui-web`.** Versioned npm package (scope as `@acryl/blends-core`), release notes and a compatibility line against the app version. Done when: a plugin outside this repo installs it and `require`s it in a real browser.
