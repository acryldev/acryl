# Feature Specification: ACR-2 - External PTY agent provider as a room peer

**Feature Branch**: `003-acr-2-pty-provider`
**Created**: 2026-08-23
**Status**: Draft
...
**Why this priority**: Native PTY is the defining ACR capability from the orientation spec (§6). This proves external agents are first-class, not subprocess hacks hidden in the UI.
...
- External agents run as native PTY processes (the approach already proven by the Development Canvas), not as DSH-native subagents. The published DSH runtime does not ship external agent backends; ACR owns the provider seam.
...
- Gap analysis §2.2: external agents must be room peers, not child-of-agent; this spec is the peer seam. Handoff between peers is ACR-3 (next).
