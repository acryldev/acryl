...
| Auto-discovery at startup | **partial, by design** | Pi loads what it finds, guarded by project trust. ACRYL's startup pass re-installs already-installed GLOBAL extensions whose source changed (only the user writes there); a changed PROJECT extension is only reported (a `git pull` can change code that would then run with the user's permissions); new folders are only ever installed by `/reload new` |
...
| `extension.json` manifest (id, API version, permissions, state schema) | **match, one file** | The `acryl` block of `package.json` (the marketplace already reads it) gains `apiVersion` (refused when newer than the runtime) and `permissions` (fixed vocabulary, shown before `/reload new`, returned by install). Declarative, not sandboxed. No second `extension.json`: two files for one package drift. State schema: not done, see `extending/state-and-persistence.md` |
| Package manager: npm and git install roots, updates, filters | **not copied, on purpose** | Pi needs its own because its extensions have no other install path. ACRYL already installs through the profile's pnpm (`dsh plugin add`) and the marketplace, so a second package manager would duplicate it. What is missing is provenance (which install came from the marketplace and which from a local folder) and one update path: that is the next design item, together with BLEND packaging (spec 036) |
