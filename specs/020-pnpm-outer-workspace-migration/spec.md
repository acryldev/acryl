...
- **FR-006**: The root workspace MUST use PNPM's isolated dependency layout unless Electron packaging evidence proves a documented compatibility exception is required. The current exception is the root hoisted linker: existing published DSH Web packages expose React 18 type references while `acryl-cli` owns React 19 types.
