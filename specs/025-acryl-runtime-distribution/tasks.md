...
- [x] T004 Replace only `scripts/publish-npm-cli.mjs` maximal dependency flattening with the proven shared TUI + Web closure.
- [x] T005 Run existing TUI/Web tests; pack the exact candidate; one clean global install passes installed `acryl --version`, `acryl tui --json`, and `acryl web --json`.
