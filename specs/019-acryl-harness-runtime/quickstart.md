...
node acryl-cli/lib/bin.js tui --profile acryl
```
...
5. Resume with `node acryl-cli/lib/bin.js tui --profile acryl --resume <session-id>` and confirm history plus continued prompting.
6. Confirm `node acryl-cli/lib/bin.js tui --profile acryl --json` stays headless.
