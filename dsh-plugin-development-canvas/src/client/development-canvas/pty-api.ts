...
function parseView(value: unknown): CanvasPtyView {
  if (!isObject(value)
    ...
    throw new Error('dsh-plugin-development-canvas: invalid canvas PTY response')
  }
  return {
    id: value.id,
    status: value.status,
...
export function createCanvasPtyApi(fetcher: FetchLike = fetch): CanvasPtyApi {
  return {
    ...
    async read(id) {
      ...
      const body = await readJson(response)
      if (!response.ok) throw new Error('dsh-plugin-development-canvas: canvas PTY read failed')
      return parseView(body)
    },
    async write(id, data) {
      ...
      if (!response.ok) throw new Error('dsh-plugin-development-canvas: canvas PTY write failed')
    },
    async resize(id, cols, rows) {
      ...
      if (!response.ok) throw new Error('dsh-plugin-development-canvas: canvas PTY resize failed')
    },
    async close(id) {
      ...
      if (!response.ok) throw new Error('dsh-plugin-development-canvas: canvas PTY close failed')
    },
  }
}
