...
function parseProfile(value: unknown): DesktopProfileView {
  if (!isObject(value)
    ...
    throw new Error('acryl-desktop: invalid profile settings response')
  }
...
export function parseDesktopSettingsView(value: unknown): DesktopSettingsView {
  if (!isObject(value)
    ...
    throw new Error('acryl-desktop: invalid Desktop settings response')
  }
  const profiles = value.profiles.map(parseProfile)
  if (new Set(profiles.map(profile => profile.name)).size !== profiles.length) {
    throw new Error('acryl-desktop: duplicate profile in settings response')
  }
  return Object.freeze({
...
export function parseDesktopRestartAcceptance(value: unknown): DesktopRestartAcceptance {
  if (!isObject(value) || value.accepted !== true || typeof value.restartRequired !== 'boolean') {
    throw new Error('acryl-desktop: invalid Desktop restart response')
  }
...
export function parseDesktopActionAcceptance(value: unknown): void {
  if (!isObject(value)
    ...
    || value.accepted !== true) {
    throw new Error('acryl-desktop: invalid Desktop action response')
  }
}

async function readResponse(response: Response): Promise<unknown> {
  if (!response.ok) {
    throw new Error(`acryl-desktop: Desktop settings request failed (${String(response.status)})`)
  }
  try {
    return await response.json() as unknown
  } catch {
    throw new Error('acryl-desktop: Desktop settings response was not JSON')
  }
}
