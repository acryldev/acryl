...
export async function requestDesktopDirectory(
  ...
  if (!isResponse(value)) throw new Error('ACRYL received an invalid response from the system folder picker')
  return value.path
}
...
export async function requestDesktopDirectoryValidation(
  ...
  if (!response.ok) throw new Error('ACRYL could not validate the selected workspace')
  const value: unknown = await response.json()
  if (!isValidationResponse(value)) throw new Error('ACRYL received an invalid workspace validation response')
