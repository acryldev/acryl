...
export class DesktopProfileService extends Service implements DesktopProfiles {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopProfileServiceBootstrap) {
    ...
    ctx.effect(
      () => () => { this.disposed = true },
      'acryl-desktop: profile service lifetime',
    )
  }
  ...
  create(name: string): DesktopProfileSummary {
    ...
    if (this.bootstrap.create === undefined) {
      throw new Error('acryl-desktop: profile creation is unavailable')
    }
  ...
  async delete(name: string): Promise<void> {
    ...
    if (this.bootstrap.delete === undefined) {
      throw new Error('acryl-desktop: profile deletion is unavailable')
    }
  ...
  private committedSelectionError(name: string): Error {
    return new Error(
      `acryl-desktop: profile ${JSON.stringify(this.committedName)} is already pending; `
  ...
  private assertActive(): void {
    if (this.disposed) throw new Error('acryl-desktop: desktopProfiles service disposed')
  }
}
