...
import type { DesktopLocale, DesktopPlatform } from '../shell/runtime.ts'
import {
...
export class ElectronWorkspaceAdmission {
  ...
  async pickDirectory(): Promise<string | null> {
    if (!this.options.canPickDirectory) {
      throw new Error(`acryl-desktop: native workspace picker is unavailable on ${this.options.platform}`)
    }
  ...
  async validateDirectory(path: string): Promise<boolean> {
    ...
    this.options.logError(`acryl-desktop: unsafe workspace volume: ${formatWindowsVolumeConcern(decision.concern)}`)
    ...
    if (decision.action === 'confirm') {
      const result = await this.options.showMessageBox({
        ...
        detail: zh
          ...
          : `Disconnecting the drive while ACRYL is running can break commands or plugin operations. Keep it connected.\n\n${path}`,
      ...
      this.options.logError(`acryl-desktop: workspace volume decision=${accepted ? 'confirmed' : 'cancelled'} path=${path}`)
      return accepted
    }
    ...
    await this.options.showMessageBox({
      ...
      message: zh
        ? `${decision.concern.fileSystem ?? '当前文件系统'} 不能安全用作 ACRYL 工作区。`
        : `${decision.concern.fileSystem ?? 'This filesystem'} cannot safely host a ACRYL workspace.`,
      detail: zh
    ...
    this.options.logError(`acryl-desktop: workspace volume decision=blocked path=${path}`)
    return false
  }
