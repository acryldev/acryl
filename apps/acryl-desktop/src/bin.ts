/** Headless-safe npm launcher for the ACRYL Electron executable. */
...
export const DESKTOP_CLI_HELP = `Usage: acryl-desktop [options]

Launch ACRYL with the selected Web-capable profile.
...
export async function runDesktopCli(
  ...
  try {
    ...
    process.stderr.write(`acryl-desktop: ${cause instanceof Error ? cause.message : String(cause)}\n`)
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  void runDesktopCli(process.argv.slice(2)).then(
    ...
    cause => {
      process.stderr.write(`acryl-desktop: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`)
      process.exitCode = 1
    },
  )
}
