...
import type { DesktopProfileSummary } from '../profile/profile-manager.ts'
import type { DesktopProfiles } from '../profile/profile-service.ts'
import type {
  ...
  async selectProfile(
    ...
    if (restartRequired) {
      ...
      if (profile === undefined || !profile.webCapable || profile.problem !== undefined) {
        throw new Error(`acryl-desktop: profile ${JSON.stringify(name)} is not selectable`)
      }
