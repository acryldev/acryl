...
import { formatDesktopExitCode } from '../diagnostics/desktop-logger.ts'
...
export class ElectronShellGeneration {
  ...
  async mount(beforeInteractive?: () => void): Promise<void> {
    if (this.mounted || this.window !== undefined) {
      throw new Error('acryl-desktop: native shell generation is already mounted')
    }

    const { platform, spec } = this.options
    const icon = nativeImage.createFromPath(spec.iconPath)
    if (icon.isEmpty()) {
      throw new Error(`acryl-desktop: failed to load application icon ${spec.iconPath}`)
    }
    ...
    const rendererGone = (_event: Electron.Event, details: Electron.RenderProcessGoneDetails): void => {
      ...
      this.options.logError(`acryl-desktop: ${detail}`)
    ...
    const loadFailed = (
      ...
      this.options.logError(`acryl-desktop: renderer failed to load (${errorCode}: ${errorDescription})`)
      ...
      try {
        ...
        if (target.protocol === 'https:' || target.protocol === 'http:' || target.protocol === 'mailto:') {
          void shell.openExternal(target.href).catch((cause: unknown) => {
            this.options.logError(`acryl-desktop: failed to open external link: ${cause instanceof Error ? cause.message : String(cause)}`)
          })
        }
      } catch {
