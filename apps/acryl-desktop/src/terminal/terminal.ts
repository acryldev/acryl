...
import { desktopTrayLabel } from '../shell/tray-locale.ts'
...
export function apply(ctx: Context): void {
  if (ctx.desktopRuntime.platform === 'linux') {
    throw new Error('acryl-desktop: the packaged terminal is supported on macOS and Windows')
  }
  ctx.effect(() => {
  ...
  }, 'acryl-desktop: packaged terminal tray command')
}
