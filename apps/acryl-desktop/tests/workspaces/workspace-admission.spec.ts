...
} from '../../src/workspaces/workspace-admission.ts'
...
describe('Electron workspace admission', () => {
  ...
  it('blocks unsupported storage and records the decision', async () => {
    ...
    expect(options.logError).toHaveBeenLastCalledWith(
      'acryl-desktop: workspace volume decision=blocked path=E:\\repo',
    )
  })
  ...
  it.each([
    ...
    expect(options.logError).toHaveBeenLastCalledWith(
      `acryl-desktop: workspace volume decision=${decision} path=E:\\repo`,
    )
  })
