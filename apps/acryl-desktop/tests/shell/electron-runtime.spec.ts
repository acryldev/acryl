...
import type { DesktopShellSpec } from '../../src/shell/runtime.ts'
...
vi.mock('../../src/terminal/desktop-terminal.ts', async (importOriginal) => ({
  ...await importOriginal<typeof import('../../src/terminal/desktop-terminal.ts')>(),
...
vi.mock('../../src/updates/update-download.ts', () => ({
  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'ACRYL' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `ACRYL-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('uses the native macOS frame, Dock icon, and template tray image', async () => {
    ...
    expect(electron.browserWindows[0]?.accessibleTitle).toBe('ACRYL')
  ...
  it('blocks unsupported workspace volumes without returning a risky path', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('acryl-desktop: workspace volume decision=blocked path=E:\\repo')
  })
  ...
  it('requires explicit confirmation for a removable NTFS workspace', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('acryl-desktop: workspace volume decision=cancelled path=E:\\repo')

    electron.dialog.showMessageBox.mockResolvedValue({ response: 0, checkboxChecked: false })
    await expect(runtime.validateDirectory('E:\\repo')).resolves.toBe(true)
    expect(logger.error).toHaveBeenCalledWith('acryl-desktop: workspace volume decision=confirmed path=E:\\repo')
  })
  ...
  it('logs renderer crashes with the Windows exception code', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'acryl-desktop: renderer process gone (reason: crashed, exitCode: -1073741819 / 0xc0000005)',
    )
    await release()
  })
  ...
  it('keeps external window links deny-by-default with a narrow protocol allowlist', async () => {
    ...
    await Promise.resolve()
    expect(logger.error).toHaveBeenCalledWith(
      'acryl-desktop: failed to open external link: external handler unavailable',
    )
  ...
  it('rejects an unsupported Electron platform before creating a runtime', async () => {
    ...
    expect(() => new ElectronDesktopRuntime(async () => {})).toThrow(
      'acryl-desktop: unsupported Electron platform aix',
    )
    expect(electron.browserWindowOptions).toHaveLength(0)
  })
  ...
  it('logs the renderer boot failure details for diagnostics', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'acryl-desktop: renderer boot failed (plugins: dsh-vision-router): '
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/ACRYL-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Downloads/ACRYL-2.1.0-mac.dmg',
    })
    ...
    expect(electron.dialog.showSaveDialog).toHaveBeenCalledWith(expect.objectContaining({
      defaultPath: join('/tmp/Downloads', 'ACRYL-2.1.0-mac.dmg'),
    ...
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/ACRYL-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/ACRYL-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  it('starts the downloaded Windows installer before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\ACRYL-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\ACRYL-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\ACRYL-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\ACRYL-2.1.0-windows.exe',
    })
  ...
  it('continues the update handoff when cleanup tracking cannot be persisted', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'acryl-desktop: failed to remember update installer for cleanup: read-only user data',
    )
    expect(childProcess.spawn).not.toHaveBeenCalled()
  })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\ACRYL-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/ACRYL-2.1.0-mac.dmg',
    })
