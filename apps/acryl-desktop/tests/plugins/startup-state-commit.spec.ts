...
} from '../../src/plugins/install-recovery.ts'
import {
  beginDesktopProfileStartup,
  markDesktopProfileFailed,
  readDesktopProfileState,
  selectDesktopProfile,
} from '../../src/profile/profile-manager.ts'
import { DesktopStartupStateCommit } from '../../src/startup/startup-state-commit.ts'
...
describe('Desktop startup state commit ownership', () => {
  ...
  it('quiesces the Host before recording a protected install failure', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      `acryl-desktop: plugin install plugin-a (${prepared.transactionId}) requires recovery after renderer-failed`,
    )
  ...
  it('keeps a healthy Profile when verified WAL cleanup fails', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'acryl-desktop: failed to clear verified plugin install recovery state: cleanup unavailable',
    )
  })
})
