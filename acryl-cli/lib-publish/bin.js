...
*/
function surfaceError(command) {
	if (command === "web") return /* @__PURE__ */ new Error("`acryl web` is served by the separate `acryl-web` distribution. Install it separately; the `acryl` CLI is the terminal (TUI) surface only.");
	return /* @__PURE__ */ new Error("`acryl gui` (Electron Desktop) is a separate distribution and is not wired into the `acryl` CLI package. Use the desktop installer.");
}
const defaults = {
	startDirectHost,
	runTui: runAcrylTui,
...
if (isEntrypoint()) (async () => {
	...
	while (current instanceof Error && !seen.has(current)) {
		...
		if (current instanceof AggregateError) {
			for (const error of current.errors) process.stderr.write(`acryl: ${error instanceof Error ? error.stack ?? error.message : String(error)}\n`);
