...
- ACR-1 room identity/durability plan;
- ACR-2 external-agent provider plan; and
...
ACR-1 and ACR-2 have aligned product requirements but incomplete implementation
...
to ACR-2's service/provider seam. The current hardcoded list must not become the
...
### Conditionally aligned, planning blocked: ACR-1 room
...
### Direction aligned, plan incomplete: ACR-2 agent control
...
- separate Canvas, ACR worker, runtime, and provider-session identities; and
...
ACR-3 through ACR-13 mostly state product intent and Cordis constraints but do
...
No custom ACR model-facing Tool has been implemented yet.
...
4. Finish the ACR-1 mini-design and DSH persistence reuse decision.
5. Finish the ACR-2 `acrAgentControl` contract and adapter reuse decision.
...
- `specs/002-acr-1-plugin-identity/`
- `specs/003-acr-2-pty-provider/`
