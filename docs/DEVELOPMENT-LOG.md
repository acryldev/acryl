...
- Spec 038-ui-component-library, tasks.md T028 step 3: `SidebarRow` (New Session bar of DSH `ui-sidebar`) and `ToolCallCard` (header, state dot, running sweep, error summary and IN/OUT card of DSH `ui-tool` ToolRow) extracted into `plugins/acryl-ui-web/src/client/registry/` with provenance in `registry-manifest.yml`, contract entries and a test that every class in the extracted stylesheets exists in the DSH stylesheet. 21 tests, typecheck clean. Not yet checked in a real browser.
- Spec 038-ui-component-library plan.md "Slice 7" and tasks.md T034 to T044: extended plan for the UI registry (git repo in the blends hub form D6, kind `ui-component`), `acryl ui add`, scaffold template, shadcn conversion pipeline, `@acryl/ui-web` npm publish, and Blends lock v2 in the `blends` repo. Planning only, nothing built by this entry.
- Spec 038-ui-component-library plan.md Slice 7 and tasks.md T036, T038, T041: one package/registry `@acryl/ui` for both web and tui (drop the `-web` suffix; tui folds in as a second surface under the same catalogue ids), registry repo at github.com/acryldev/acryl-ui-registry, `acryl ui add --surface web|tui`. Planning only.
- Spec 038-ui-component-library tasks.md T034 done: confirmed no engine composition auto-installs the UI-library gallery example (every pack example is already opt-in); `docs/extending/ui-library.md` now says install-and-remove as a pair. Note: my own test install of `acryl-example-ui-library` is still sitting in the user's real Web profile pending `/reload remove-stale`.
- Spec 038-ui-component-library tasks.md T035 partial: corrected the primitive count (28 real components, not ~48) and exported 15 more as re-exports with contract entries; typecheck/tests/build pass. Gallery demos and a real-browser check are still open.
- Spec 038-ui-component-library tasks.md T035: real-browser check (scratch profile copy, dark scheme) of the 15 newly re-exported primitives - all render and respond; found and fixed a demo bug (RiskConfirmation needs open/description/acknowledgeLabel, not just title+onConfirm). WebBlock/CodeBlock/JsonBlock/MarkdownText still undemoed.
- Spec 038-ui-component-library tasks.md T041: published to npm as `acryl-ui@0.3.0` (unscoped; the `@acryl` org does not exist yet, website-only to create, user chose the unscoped name for now). Real install verified in a scratch project. In-repo package stays `@acryl/ui` for internal use; publish to `@acryl/ui` and deprecate `acryl-ui` once the org exists.
- Spec 038-ui-component-library tasks.md T042: pushed a lock-v2/ui-component-kind proposal to the `blends` repo as its own spec-004 (per that repo's methodology, touching none of its pre-existing uncommitted files) - `acryldev/blends` commit `6f35b32`. Not yet accepted or implemented there.
- Spec 038-ui-component-library tasks.md T042 done: implemented lock v2 and the ui-component hub kind in acryldev/blends (not just proposed), verified against the real acryl-ui-registry (9 items, zero problems). T043 further along but not finished (CLI list/index wiring, Blueprint publish still open).
- Spec 038-ui-component-library tasks.md T035 closed: 24 of 28 DSH primitives exposed (3 more this pass: ReferenceIcon, LinkIcon, DocumentFileIcon), 4 excluded with stated reasons (brand assets, app-root coupling). Real-browser check of WebBlock/CodeBlock/JsonBlock/MarkdownText and the new icons on a scratch profile.
- Spec 038-ui-component-library: "go with the rest, all 4" closed. (1) Remaining primitives done (T035 closed at 24/28). (2) shadcn batch: Badge, Skeleton, Spinner, Alert, Separator, Progress, Avatar ported (T040), plus a real self-containment bug found and fixed (SwitchField imported another items CSS; the ingest gate now catches this class of bug). (3) contracts/categories.json: all 106 shadcnblocks.com category names (naming only), 20 with real matches. (4) Both acryldev.github.io/ui and acrylblends.github.io/ui now render the actual registry component source live (a real preview-primitives.tsx shim, not a screenshot), sidebar by category, usage snippet with a copy button. Registry now has 18 items; pushed and re-verified with a fresh clone + the blends-cli ingest each time.

## 2026-09-23 - feat: UI library T045 close-out, 26 shadcn/ui ports and a gate that catches its own regressions (spec 038-ui-component-library)

- Spec 038-ui-component-library, tasks.md T045: the remaining shadcn/ui items ported, 26 in all across batches 1-9 - Table, DirectionProvider, Marker, Message, Bubble, Pagination, NativeSelect (batch 1, `1c99248316fa096a753dabbb86afad2b4e286adc`),
  ScrollArea, InputOTP, Item, Attachment, InputGroup, FormField (batch 2, `3569101d9924ab8c8ee29db4cbd9722663465f60`), Popover and Slider (batch 3, `0d10b6eddd035a36e00546ae6bd8db4067b1e476`), Sheet (`91bd975bb7c07fd5476368440e6021513671d5d2`) and
  Drawer (`bb4b1f455f6b2a44fbb1d9202f1a6b9088aa51bb`), Command (`115aa4edf5dc11b2455ade0f74046c1a5e1bbb11`) and Combobox (`9dff3e548b0518761d1f6db41282834dd34d0a02`), ContextMenu (`67297cef294e71ff1f258babb892720da3853b43`), Carousel
  (`1ff5506270a1e32d987b1c26a35ad731e16a5129`), ResizablePanelGroup (`de0f8859644740a2a3a9295300c0ee11754aaaff`), Menubar and NavigationMenu (`2ec7c6461988957f4aa88edd722629797047e60e`), Calendar (`47e5f86fe772962488c79ca86e51baf4764d4e4d`) and
  Sidebar (`baf73e4126cf0dc1346d8ea023748eb052ab3cda`). Every external engine the items wrap is dropped and the behaviour written by hand, because a client bundle here may require only `react`, `react/jsx-runtime` and the app primitives (the built-bundle test enforces it):
  Radix (menus, popover, slider, sheet, drawer, toggle, collapsible, avatar), Base UI, vaul, cmdk, embla, recharts, react-day-picker, date-fns and react-resizable-panels. Registry now 45 shadcn-sourced items of 56.
- Four items could NOT be ported and are recorded with their evidence in `plugins/acryl-ui/docs/remaining-t045-items.md`: `chart` (recharts) and `form` (react-hook-form), each removed from scope earlier, and, from this pass, `message-scroller`
  and `questionnaire`. The last two are the same class - a registry item that is only thin wrappers (131 and 333 lines of class strings) over a primitive that carries the substance, `@shadcn/react`, which the item re-exports its own hooks from;
  `questionnaire` additionally exists only in the `base-nova` style family (the new-york-v4 slug returns the docs page, not an item) and its own source imports `@/app/(create)/components/icon-placeholder`, a private module of the docs site that is not published at all.
- Deliberate partials, all in `registry-manifest.yml` rather than left implicit: 14 of the 45 items ship a narrower core with the rest recorded - the submenu family for ContextMenu and Menubar, the portal for Sheet, Drawer, Popover and the bars (react-dom's
  `createPortal` is outside the set a bundle may require, so an ancestor with `overflow: hidden` or a transform can clip a panel), Combobox's chips and multi-select, CommandDialog, embla's plugin and event surface, react-day-picker's multi-month/dropdown/week
  numbers/extension points, react-resizable-panels' pixel sizes and imperative handles, NavigationMenu's Indicator and morph, the Sidebar's mobile Sheet, cookie and tooltips, and vaul's velocity dismissal. Every one names what is missing and what replaces it.
- Gate hardening, each guard proven by degenerate input before it was trusted: `scripts/verify-manifest.mjs` (parses the YAML and requires name/origin/from/changed per entry - it caught a fifth "nested mappings are not allowed in compact mappings" wording trap
  in under a second, after four had shipped), `scripts/verify-artifact.mjs` (fails if `lib/client.js` is missing or older than any source, which a silently-skipped `describe.skipIf` had been hiding - the stale bundle had caused React #130 in the running app while
  source tests passed), a seed-freshness guard in `validate-registry.mjs` (it had reported "42 item(s) passed" against a stale seed), a spec test for the same staleness, and a `check` chain that is `&&`-only with no pipes, because a `| tail` had been masking a
  non-zero build exit twice. `8122a2044ebf91464bcd7e778e9bf202f7359445`, `ef6d859984af905ae1cbf081db24e8e481763af6`.
- Design written down instead of repeated per item: `docs/pattern-anchored-listbox.md` (the anchored-listbox shape: root-owned open state, real focusable trigger with aria-expanded/controls, positioned not portalled with viewport clamping and re-measure, focus only once
  visible, Escape and outside pointerdown to dismiss, DOM-order keyboard walk read at key time, aria-activedescendant) - cited explicitly from every item that restates it, and never made an importable module, because the ingest gate rejects cross-item imports and
  self-containment is absolute: `8e8fe198fa653bb0b1c2030b636c5f95452e2dff`. Handover for the remaining items (pipeline, per-item deltas, browser-check traps) in `plugins/acryl-ui/docs/remaining-t045-items.md`, `08a929deeacd51a40dabd90e0714b0e550c9526c`,
  and Carousel's design decided in advance of its execution: `da0e48c7d023785cb2265b75df027915029563ad`.
- Real-browser verification was where the defects were, one or more per batch, and each is now a line in the traps list rather than a story: ScrollArea narrowed to `className` so its viewport grew instead of scrolling; Popover's focus never landing in a panel that
  starts `visibility: hidden` (`85982b1aebb0e6a198f27a6ebca38afaec730491`); Combobox's Enter closing the panel without setting the value, because selection had been delegated to an item's `onSelect` - cmdk's shape, not a combobox's; ContextMenu's `display: contents` trigger
  being boxless and unfocusable; InputGroup transitioning a property that a `:has()` rule set, which the operator's own Chrome 153 repro showed is an engine split against the 149 the rig runs (`5aec931ee07bb9283badb26a31257e621508ffee`); Carousel's slides measuring 426px
  inside a 410px viewport because the module never declared `box-sizing` (this app ships no global reset); Calendar's roving focus landing on a disabled day, whose button refuses focus, so the state moved and the reader's focus did not - it now steps past anything the
  predicate disables; and the Sidebar's grab strip being clipped away exactly when it is needed, because it lives inside the rail's inner box. Two more findings were the harness's own: a parent cannot measure a child in the effect pass that mounts it (the navigation menu's
  shared surface measured a 2x2 box until a resize), and `:nth-of-type` on a trigger that sits alone inside its own wrapper matches nothing, so a check can report a bug that is not there.
- Verification: `pnpm run check` (manifest, build, artifact freshness, seed + ingest gate, typecheck, 72 tests) exits 0, and each of the 26 ports has a gallery demo whose behaviour was driven in a real browser over CDP - 16/16 for Carousel, 18/18 for
  ResizablePanelGroup, 23/23 for the two bars, 15/15 for Calendar, 10/10 for Sidebar - including real pointer drags, real modifier shortcuts, roving focus and the states the contracts promise. The scratch rig patched all eleven `client.js` copies in the staged profile and
  drove Chrome-for-Testing over CDP with Node's global `WebSocket`; the traps (stop the previous server first, a source change needs a rebuild, `element.click()` produces no `pointerdown`, Escape also closes the surrounding Settings view) are in the handover doc.
...
## 2026-09-26 - Split divider, Review tab and Checks tab (spec 040)

- **What changed:** three more pieces of the three-pane ADE in `acryl-workspace`. The optional split now has a draggable, keyboard-operable divider (arrow keys, double click resets to half) whose width is remembered. A Review right-pane tab lists every line comment sent to the agent, per worktree, with resolve, reopen and remove, persisted in the renderer. A Checks right-pane tab lists the worktree's package scripts and runs one in a terminal tab of that worktree.
- **Design notes:** Review only keeps the reviewer's own bookkeeping; the comment itself stays durable in the chat session log. Checks adds one read-only Host route (`git/checks`) that reads `package.json` scripts and the lockfile-named package manager, and only offers script names matching a plain pattern, because the name is typed into a shell on the user's behalf; the command line is built from the validated name and the project's manager, never from free text. Both tabs are dependency-gated child plugins like Changes (PENDING without the right sidebar, unwind together on disposal).
- **Verification:** package check green (218 tests) including a real-Cordis lifecycle test per tab, real temporary directories for the checks reader and route, and render tests. Not automated: the canvas effect that opens the terminal for a run and the divider drag with a real pointer; both need the running window.
- **Commits:** `e771d9e` (divider), `12fa3f5` (Review tab), `0b66deb` (Lavish page progress section and ledger), `dfef27c` (Checks tab).
- **Lavish page:** `specs/040-agentic-multiplexer-ade/design/ux-2-where-we-go.html` now opens with a Progress section (built vs next) and updated status chips, and is kept current at each checkpoint.

## 2026-09-26 - Files tab and CodeMirror editor, domain layout, single right-panel opener (spec 040)

- **Single opener:** the canvas tab strip carries the always-present right-panel toggle, and upstream's own header opener (`data-sidebar-right-expand`) is hidden inside the canvas, so there is one icon, not two. `e0a8978`.
- **Code organization:** on the owner's direction to follow the DDD module rules (modules named for model meaning, discoverable from the directory tree, no generic folders), `plugins/acryl-workspace` moved from a flat `src/client/workspace/` (about 40 files) to domain folders: `git`, `pty`, `files`, `http` on the Host; `canvas`, `worktrees`, `projects`, `sessions`, `terminal`, `git`, `changes`, `diff`, `review`, `checks`, `files` on the Client; tests mirror them. Done with a scripted move that rewrites every relative import, then verified by the full package check (218 tests unchanged). A README documents the layout. `342723e`.
- **Files tab and editor:** a Files right-panel tab (lazy tree, name filter over opened folders) and an editor tab built on CodeMirror 6 (MIT), chosen over Monaco for size and modularity and because `@codemirror/language-data` covers the major languages on demand. The Host adds three routes (`files/tree`, `files/read`, `files/write`); this is the only code in the package that writes user files, so paths are confined (git worktree root only, relative, no `..` or `.git`, real path inside the worktree, symlinks that escape are refused), files over 2 MB are refused, saves are atomic (temp file then rename, mode kept) and answer 409 when the file changed on disk since it was read. The editor keeps unsaved drafts in the tab state (never in storage) with the disk version they are based on, and warns before a stale draft could overwrite newer disk content. `86fa805`.
- **Dependency install note:** `pnpm install` for the whole workspace hung during resolution on this machine. The six CodeMirror packages were resolved in a scratch project and their 53 lockfile entries merged into `pnpm-lock.yaml`, then verified with `--frozen-lockfile` and linked. The client bundle is a single 3.3 MB file because the Module Loader evaluates one file, so on-demand language packs are inlined (`inlineDynamicImports`); see the package README for the option to curate languages.
- **Verification:** 263 tests pass, including real temporary git repositories for the Host (escape attempts, conflicts, atomic write, routes), the real CodeMirror editor in jsdom (edit, save key, on-demand highlighting), and pane render tests. Not automated: how the editor looks and feels in the running window.
- **Not done:** creating, renaming and deleting files; repo-wide search; the same reorganization for `apps/acryl-desktop/src` follows in the next entry.

## 2026-09-26 - apps/acryl-desktop reorganized by bounded context

- **What changed:** `apps/acryl-desktop/src` (85 flat files) and its tests (96 flat files) are now grouped by the context each file belongs to: `shell`, `profile`, `plugins` (with `lifecycle` and `architecture`), `startup`, `diagnostics`, `updates`, `settings`, `terminal`, `workspaces`, `windows`, `runtime`, and the client half by `layout`, `settings`, `plugin-lifecycle`, `plugin-architecture`, `workspaces`. Tests mirror the folders; packaging and release tests stay at the tests root. `apps/acryl-desktop/docs/code-layout.md` documents the map. `4fbe5b5`.
- **Hazard found and handled:** eleven modules locate siblings through `import.meta.url` (the flat `lib/` output, the package root, `native-ui/*.html`), so their depth in `src/` is load-bearing: in `lib/` every entry is a sibling. A first pass that moved them too made unit tests fail, and would have broken the packaged app while other tests passed. They were moved back to the `src` root and are listed in `code-layout.md` with the condition for moving them later (a depth-independent locator plus a packaged-runtime test). A scripted move also rewrote quoted relative strings inside test fixtures (`'./index.mjs'` became `'../../index.mjs'`); those were restored by an audit of every changed non-import line.
- **Config coupling:** tsdown entry paths, `package.json` `exports[*].types` (now `lib/types/<folder>/<file>.d.ts`), the test tsconfigs (explicit test names) and `tests/package.spec.ts` expectations were updated. The client tsconfigs list `advanced-shell.ts` first: TypeScript's file order changes how a circular Cordis `Context` type resolves, and with another first file `ctx.effect` vanishes from the type.
- **Verification:** all five tsconfigs clean, 858 desktop tests pass (4 skipped as before), the tsdown build into a scratch directory produces the same set of entry files as before, and every `exports` types target exists in a scratch declaration emit. Not run: the real `pnpm run build` and the `verify:loader`, `verify:profile` and `verify:closure` smokes, because they rewrite `lib/` under the running dev app. Run `corepack pnpm --filter acryl-desktop run check` after stopping the dev app to close that gap.

## 2026-09-26 - correction: the ADE must be one implementation for Web and Desktop

- **Finding:** the workspace work of 2026-09-24 to 2026-09-26 was composed as a Desktop-only row in `apps/acryl-desktop/src/profile.ts` with its layout slots in the Desktop app, so Web has none of it. That contradicts decisions already on record: `docs/ACRYL-RUNTIME-SURFACE-CONTRACT.md` (implement once, surfaces render), the 2026-08-31 shared coding capability composition (`ad6a510`, seam `runtime/acryl-harness-runtime/src/coding-capabilities.ts` with a `surfaces` field per capability), and spec 034 (one implementation, per-surface declarations, parity gate).
- **Decision (owner):** users run both surfaces, so the ADE is built once in shared space, and Agent Control (spec 041) is for both Web and Desktop too.
- **Recorded in:** spec 040 (new "Surface sharing" section, requirements SH-001 to SH-006, Phase 7 tasks T070 to T076 that come before further features), spec 041 (surfaces section, US4 raised to P1, FR-013, plan sharing section, tasks TS01 to TS05), the Lavish page (decision 5 and a surface-gap banner). No code changed in this entry.

## 2026-09-26 - the workspace and shell are now one implementation for Web and Desktop (spec 040 Phase 7)

- **Composition:** `runtime/acryl-harness-runtime/src/coding-capabilities.ts` gained `workspace` (`surfaces: ['desktop','web']`, `requiresPackages: ['acryl-workspace']`) and `advanced-shell` (the `ui-layout` off, `ui-sidebar` and `ui-conversation` on toggles, composed by `createAcrylShellCapabilityPatches(surfaces, mode)`). Desktop's hand-written workspace row and inline toggles are gone; Desktop passes its user-chosen mode, Web is always `advanced`. Both Web boot paths (engine host and the older `bootAcrylWebProfile`) compose through the same data and link the package into the profile. `c6d0543`.
- **Shell moved:** the advanced frame, layout state and service, slot declarations and theme presenter moved from `apps/acryl-desktop/src/client` to `plugins/acryl-workspace/src/client/shell`, with a shared `environment.ts` (Electron's URL markers, or web when absent) and `chrome-metrics.ts` (native chrome, only reserved on macOS and Windows; a test compares it with Desktop's `window-chrome.ts`). Desktop's client keeps only its native parts. `2480aff`.
- **Interim breakage avoided by one commit apart:** `c6d0543` alone would have left Web blank (stock frame off, no ACRYL frame); `2480aff` follows it directly.
...
## 2026-09-26 - parity audit: plugin administration shared, a permanent Web and Desktop parity gate, CI blind spots closed

- **Audit (measured):** the real Desktop profile and the real Web engine were composed and their Loader rows diffed. Desktop had nine rows Web lacked, all in the monolithic `acryl-desktop` package, and one user-facing feature that was shareable: Settings > Plugins Lifecycle and Architecture.
- **Extraction:** those became `plugins/acryl-plugin-admin`, two Cordis plugins: a Host plugin (the architecture route from the web server alone; the lifecycle routes as a dependency-gated child that mounts when a surface has published `ctx.acrPluginLifecycle`, which both do) and a Client plugin contributing the two `settings.plugins.tab` entries. The renderer-facing projection is one `PluginLifecycleView`, which Desktop's plugin controller now reuses. Paths moved to `/api/acryl-plugin-admin/...`; the restart callback that only Desktop used was dropped. Composed for both surfaces by a `plugin-admin` capability; the real Web boot serves its routes and the page loads without a plugin error. `623b24b`.
- **Gate:** `apps/acryl-desktop/tests/surface-parity.spec.ts` composes both real surfaces and fails when a row exists on only one without a listed reason; every Desktop-native row is listed with why (see `research.md`). The remaining Web gaps are named: notifications, diagnostics, Blends.
...
## 2026-09-26 - owner review of the Web surface: terminal rebuild, tab strip

- **Tab strip** (`70447753f3c778bad89dc73f4152a505453bcacb`, `0daf74f7efebb41c9b958dbfa17fa3e08e5b3a5c`): sideways scroll with edge fades and the active tab kept in view; an "N agents running" pill that opens the live board.
- **Terminals** (`3c3726513963204a7c00d162b51a4dde40fa5fa7`): the owner reported glitchy and sometimes broken terminals. Root causes found by reading the code, not guessed: the Host kept only a 64KB sliding string and the browser re-fetched all of it every 100ms, so once it slid the client cleared the terminal and rewrote a tail that began mid escape sequence; every keystroke was its own HTTP request and could arrive out of order; each tab switch destroyed the terminal and rebuilt it from that tail; and the page carried a hand-copied, partial xterm stylesheet (no row, cursor or decoration rules). Replaced with an ordered WebSocket stream (`ws`, registered through the web server's upgrade routes, same-origin loopback check), a cursor-addressed 1MB scrollback so a reconnect resumes with no gap or repeat, terminal sessions owned by the canvas so a tab switch only moves the element, xterm's real stylesheet inlined by a small bundler plugin, and a UTF-8 truecolor environment. Verified against the real Web engine with a real PTY over the WebSocket. Not yet verified in a browser with a full-screen TUI (T105).
- **Tabs** (`99d80aadbbb0e857b2d87772a0963d1b71bee556`): renamable in place, agent icon badges, cleaner flat style, and a "+" menu whose agent list the user configures. Custom user-defined agents need a Host allowlist design first (T098).
- **Ledger:** every idea from the owner's review is recorded as T090 to T110 in `specs/040-agentic-multiplexer-ade/tasks.md`, Phase 9.

## 2026-09-26 - Phase 9: custom agents, terminal fidelity, layout and chrome

Design: `specs/040-agentic-multiplexer-ade/design-phase-9.md`. Tasks T090 to T117 in `tasks.md`.

- **Terminal fidelity** (`acb60e5f1080d0ded130f09280ff6497281561de`, fix `c4fe6fd5fb0cca2739858a35c1c7a86bd47643c1`): the Host keeps a headless screen model per terminal (`@xterm/headless` plus the serialize addon), so a view that attaches without a usable cursor gets the exact screen, including a full-screen program's alternate screen. Terminals start at the pane's estimated size. WebGL renderer with DOM fallback. Tested against a real PTY: alternate screen, resize, a 30,000-line burst. **Mistake recorded:** the first push of this was broken. `@xterm/headless` is CommonJS and Node's ESM loader rejects its named export; vitest hides that, the real Web boot test caught it, but my command chain pushed anyway. Fixed within minutes (`c4fe6fd5fb0cca2739858a35c1c7a86bd47643c1`). The commit-and-push step is now gated on the real-boot test passing.
- **Custom agents** (`b2cd76b364b8b7ca1c208fde01a33df23d236e07`): a catalog in the user's own ACRYL home, never in a worktree; definitions are validated (no shell, no metacharacters, palette badges, argument arrays), refused with a message that says what to fix, and started by id only. The form shows the exact command line before saving. Adversarial tests and a real-boot test that starts a custom agent.
- **Chrome** (`ec3064242a96bfa1e70ad3d4a507057ec9a10e99`): status line, agent icons on worktree rows, +/- totals in Changes, agent-finished notices that lead back to the tab. Then tab context menu, running marker, and hideable tab types.
- **Terminal tabs survive a page reload** (`c35ab34cc41a9611d0658d434f1793b7e1627497`): saved by Host terminal id, reattached to the exact screen; a terminal the Host no longer has closes its tab.
- **Structure** (Clean Architecture and DDD audit): the git service split by use case over one confined runner; scratch panes moved into their domain folders; node-pty behind an injected adapter; the terminal handle on tabs renamed `terminalId`.
- **Not verified in a real browser:** everything in this entry. The Web boot with a real PTY, WebSocket and a custom agent is covered by `apps/acryl-web/tests/workspace-shared.spec.ts`.
