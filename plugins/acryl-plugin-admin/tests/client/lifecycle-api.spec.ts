...
} from '../../src/client/lifecycle/plugin-lifecycle-api.ts'
...
describe('plugin lifecycle client API', () => {
  ...
  it('merges the current Client Loader phase into the Host snapshot', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('/api/acryl-plugin-admin/lifecycle', expect.objectContaining({
      method: 'GET',
      cache: 'no-store',
    }))
  })
  ...
  it.each([
    ['enable', 'include:acryl-development-canvas', '/api/acryl-plugin-admin/lifecycle/enable'],
    ['disable', 'include:acryl-development-canvas', '/api/acryl-plugin-admin/lifecycle/disable'],
    ['reload', 'include:acryl-development-canvas', '/api/acryl-plugin-admin/lifecycle/reload'],
  ...
  it('supports reload-all with an exact empty request', async () => {
    ...
    const api = createPluginLifecycleApi(loader(), fetcher, reloadPage)
    await api.reload()
    expect(fetcher).toHaveBeenCalledWith('/api/acryl-plugin-admin/lifecycle/reload', expect.objectContaining({
      body: '{}',
    }))
  })
