...
import { parseSavedWorkspace, serializeWorkspace, STORAGE_KEY, type StorageLike } from '../../src/client/canvas/persistence.ts'
import { startWorkspacePersistence } from '../../src/client/canvas/persist.ts'
...
const shellStub = (): WorkspaceShellState => new WorkspaceShellState({
  ...
  search: async (path, query, mode) => ({ path, query, mode, hits: [], truncated: false }),
