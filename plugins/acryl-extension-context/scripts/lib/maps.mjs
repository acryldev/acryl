...
export function renderMountPoints(slots, hostSeams) {
  ...
  const lines = [
    ...
    'shows (chat, tool cards, status) is built in. Example: `example-plugins/packages/tui-command-basic/`; doc: `extending/tui-command.md`.',
    '',
...
/** Value exports of the shared web UI primitives (what `require('@deepseek-ai/dsh-client-ui-primitives')` returns). */
export function collectPrimitiveExports(repo) {
  const text = read(join(repo, 'deepseek-harness/packages/client/ui-primitives/src/index.ts'))
  const names = new Set()
  for (const m of text.matchAll(/^export \{([^}]*)\} from/gmu)) for (const n of m[1].split(',')) { const name = n.trim().split(/\s+as\s+/u).pop(); if (name) names.add(name) }
  const iconsFile = read(join(repo, 'deepseek-harness/packages/client/ui-primitives/src/icons/index.tsx'))
  const icons = [...iconsFile.matchAll(/export (?:function|const) (Icon[A-Za-z0-9]+)/gu)].map(m => m[1])
  return { components: [...names].sort(), icons: icons.sort() }
}

export function renderUiComponents(prims, tui) {
  return [
    ...
    '',
    `${prims.components.length} exports:`,
    '',
    prims.components.map(n => `\`${n}\``).join(', '),
    '',
    `${prims.icons.length} icons (React components, e.g. \`IconFolderClose16\`): ${prims.icons.slice(0, 60).map(n => `\`${n}\``).join(', ')}${prims.icons.length > 60 ? ', ...' : ''}`,
    '',
...
/** Every event declared in an `interface Events` block (declare module '@deepseek-ai/cordis'): name, dispatch mode, first doc line, package. */
export function collectEvents(repo, index) {
  const roots = [join(repo, 'deepseek-harness/packages'), join(repo, 'apps'), join(repo, 'plugins'), join(repo, 'runtime')]
  const files = []
  for (const root of roots) walk(root, 7, (file, name) => { if (/\.(ts|tsx)$/u.test(name) && !/\.(test|spec)\./u.test(name)) files.push(file) })
  const packageOf = file => {
    let best
  ...
  for (const file of files) {
    const text = read(file)
    const block = /interface Events \{/gu
    let m
    while ((m = block.exec(text)) !== null) {
      let depth = 1
      let i = block.lastIndex
      ...
      while (i < text.length && depth > 0) { if (text[i] === '{') depth += 1; else if (text[i] === '}') depth -= 1; i += 1 }
      const body = text.slice(start, i - 1)
      const entry = /(?:\/\*\*([\s\S]*?)\*\/\s*)?'([A-Za-z0-9:/._-]+)'\s*\(/gu
      let e
      while ((e = entry.exec(body)) !== null) {
        const docLines = (e[1] ?? '').split('\n').map(l => l.replace(/^\s*\*?\s?/u, '').trim()).filter(Boolean)
        const what = docLines.find(l => !l.startsWith('@')) ?? ''
        const mode = /@mode\s+(\w+)/u.exec(e[1] ?? '')?.[1] ?? ''
        if (!events.has(e[2])) events.set(e[2], { name: e[2], mode, what: what.replace(/\s+/gu, ' '), declaredBy: packageOf(file) })
      }
    }
  }
...
export function renderEvents(events) {
  return [
    ...
    '| --- | --- | --- | --- |',
    ...events.map(e => `| \`${e.name}\` | ${e.mode} | ${cell(e.what)} | ${cell(short(e.declaredBy))} |`),
    '',
  ].join('\n')
}

// ---------------------------------------------------------------- slot contracts (from the harness's own client catalog)

const noDash = text => String(text ?? '').replaceAll('—', '-').replaceAll('–', '-')

/** Load the harness's generated client slot catalog (data only, TypeScript type-stripped by Node). */
export async function loadSlotCatalog(repo) {
  const file = join(repo, 'deepseek-harness/packages/extensions/cordis-client-runner/src/client/slot-catalog.ts')
  if (!existsSync(file)) return undefined
  const mod = await import(file)
  return { notes: mod.CLIENT_NOTES ?? [], slots: mod.CLIENT_SLOT_API ?? [] }
}

export function renderSlotContracts(catalog) {
  const out = [
    ...
    '## Harness notes',
    '',
    ...catalog.notes.map(note => `- ${noDash(note).replace(/\n/gu, ' ')}`),
    '',
    `## The ${catalog.slots.length} slots`,
    '',
  ]
  for (const slot of catalog.slots) {
    out.push(`### \`${slot.key}\``, '', `Kind \`${slot.kind}\`, scope \`${slot.scope}\`. ${noDash(slot.summary)}`, '')
    if (slot.doc && slot.doc.trim() !== slot.summary.trim()) out.push(noDash(slot.doc).trim(), '')
    if (slot.registerOptions?.length) {
      out.push('Register options:', '')
      for (const o of slot.registerOptions) out.push(`- \`${o.name}\` (${o.requirement}, \`${cell(o.type)}\`): ${noDash(o.doc).replace(/\n/gu, ' ')}`)
      out.push('')
    }
    if (slot.ownerProps?.length) out.push('Props your component receives:', '', '```ts', noDash(slot.ownerProps.join('\n\n')).trim(), '```', '')
    if (slot.keyDomain) out.push(`Keys: ${noDash(slot.keyDomain)}`, '')
    if (slot.occupants?.length) out.push(`Already occupied by: ${slot.occupants.map(o => noDash(o)).join('; ')}. Replace risk: \`${slot.replaceRisk}\`.`, '')
    if (slot.example) out.push('Example:', '', '```js', noDash(slot.example).trim(), '```', '')
  }
  return out.join('\n')
}
