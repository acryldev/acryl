...
4. Call acryl_install_plugin with the same directory: it updates in place. Read the result: if it has a "warning" the
   host code change needs an app restart unless the plugin uses the hot shim
   ({{pack}}/examples/packages/lifecycle-function-hot-shim/); if it has a "next" note, tell the user to reload.
