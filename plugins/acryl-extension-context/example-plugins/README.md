...
| [lifecycle-function.hot-shim](packages/lifecycle-function-hot-shim/) | lifecycle-function | LEGACY: a hand-written hot shim. The installer now reloads host code automatically; this pattern is only needed if you bypass acryl_install_plugin. | tui web desktop | delivery.local-live |
...
| [ui-components.shared-primitives](packages/client-ui-components/) | ui-components | build UI from the shared primitives (Button, Modal, Input, Switch, Tag) via require, in the root-scoped sidebar.footer.action slot. | web desktop | extending.ui-components, extending.client-slot |
| [prompt-assemble-hook.basic](packages/prompt-assemble-hook/) | event-hook | hook prompt construction on the system-prompt/assemble waterfall: await next(), change the assembly, return it. | tui web desktop | extending.event-hook |
| [lifecycle-hooks.observer](packages/lifecycle-hooks-observer/) | event-hook | observe the agent lifecycle on the pre-step, request and llm/stream waterfalls: always call next() and return its result. | tui web desktop | extending.event-hook |
| [client-tool-view.custom-card](packages/client-tool-view/) | client-slot | a tool plus its own conversation card via the keyed tool.call.toolview slot (renders running and settled calls). | web desktop | extending.client-slot |
| [tool-policy-hook.deny](packages/tool-policy-hook/) | event-hook | a tool policy on the tools/pre-execute waterfall: allow, deny with a reason, or ask; return next() for everything else. | tui web desktop | extending.event-hook |
| [client-chat-message-action.bookmark](packages/client-chat-message-action/) | client-slot | the additive chat-message seams: an action under each finished assistant message (assistant-actions list slot), plus notes on turnTail and chat-node takeovers. | web desktop | extending.client-slot |
| [state.host-store](packages/state-host-store/) | host-route | Keep state on the host (a JSON file under the DSH home) behind an authenticated RPC channel, so Web and Desktop share it and it survives clearing the browser. | web desktop | extending.state-and-persistence, extending.host-route |
| [state.workspace-file](packages/state-workspace-file/) | tool | Keep project notes in the workspace as .acryl/notes.md via tools, so the agent and the user can read them and git can track them. | tui web desktop | extending.state-and-persistence, extending.tool |
| [ui-library.gallery](packages/client-ui-library/) | client-slot | Build a screen from the ACRYL UI library (acryl-ui-web): cards, fields, settings rows, dropdown, segmented tiles, tabs, dialog, color roles and slot helpers. | web desktop | extending.ui-library |
| [ui-library.tui-gallery](packages/tui-ui-library/) | tui-contribution | Build a terminal screen from the ACRYL UI library (acryl-ui-tui): cards, fields, settings rows, select, segmented, tabs, dialog, colors from the shared roles and the tuiTheme service. | tui | extending.ui-library, extending.tui-components |
